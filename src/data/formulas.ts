...
export const pythonTemplates: readonly TemplateCategory[] = [
  ...
  },
  {
    ...
    templates: [
      ...
      {
        name: 'One-Way ANOVA (f_oneway)',
        code: 'stats.f_oneway(group1, group2, group3)',
        description: 'Tests H₀: all group means are equal. Returns (F-statistic, p-value).',
        params: ['group1, group2, ... = arrays of observations per group'],
        gotcha: 'Significant result only tells you "at least one differs" — run Tukey HSD to find which pairs.',
      },
      {
        name: 'One-Way ANOVA (OLS method)',
        code: 'from statsmodels.formula.api import ols\ndf_long = df.melt(var_name="group", value_name="value")\nmodel = ols("value ~ group", data=df_long).fit()\nsm.stats.anova_lm(model)',
        description: 'OLS-based ANOVA gives the full table: df, SS, MS, F, p-value.',
        params: ['df.melt() converts wide → long format', 'ols formula: "value ~ group"'],
      },
      {
        name: 'Tukey HSD (post-hoc)',
        code: 'from statsmodels.stats.multicomp import pairwise_tukeyhsd\ntukeyhsd = pairwise_tukeyhsd(\n    endog=df["value"], groups=df["group"], alpha=0.05\n)\ntukeyhsd.summary()\ntukeyhsd.plot_simultaneous()',
        description: 'Pairwise comparisons after significant ANOVA. Shows which specific pairs differ.',
        params: ['endog = response variable (values)', 'groups = group labels', 'alpha = significance level'],
      },
      {
        name: 'Two-Way ANOVA',
        code: 'model = ols("Score ~ Treatment + Gender + Treatment:Gender",\n    data=df).fit()\nsm.stats.anova_lm(model)',
        description: 'Tests two factors and their interaction simultaneously.',
        params: ['A + B = main effects', 'A:B = interaction term'],
        gotcha: 'If interaction p > α, the interaction is not significant — the two factors act independently.',
      },
      {
        name: 'ANOVA Assumption: Shapiro-Wilk on residuals',
        code: 'residuals = model.resid\nstat, p = stats.shapiro(residuals)',
        description: 'Tests normality of ANOVA residuals. H₀: residuals are normally distributed.',
      },
      {
        name: 'ANOVA Assumption: Levene\'s test',
        code: 'stats.levene(group1, group2, group3)',
        description: 'Tests equal variances. H₀: variances are equal across groups.',
        gotcha: 'If p < α, variances are unequal — consider Welch ANOVA or Kruskal-Wallis.',
      },
      {
        name: 'Kruskal-Wallis (non-parametric ANOVA)',
        code: 'stats.kruskal(group1, group2, group3)',
        description: 'Non-parametric alternative to One-Way ANOVA. Uses ranks, not raw values.',
        params: ['group1, group2, ... = arrays per group (can be unequal sizes)'],
        gotcha: 'Tests distributions, not means. If distributions have similar shape, interpreted as testing medians.',
      },
      {
        name: 'Dunn\'s test (post-hoc for Kruskal-Wallis)',
        code: 'import scikit_posthocs as sp\nsp.posthoc_dunn(df, val_col="value",\n    group_col="group", p_adjust="holm")',
        description: 'Pairwise comparisons after significant Kruskal-Wallis. Returns matrix of adjusted p-values.',
        params: ['val_col = column with values', 'group_col = column with group labels', 'p_adjust = correction method'],
      },
      {
        name: 'Chi-Square Test of Independence',
        code: 'from scipy.stats import chi2_contingency\nmatrix = np.array([[a, b], [c, d]])\nchi2, p, dof, expected = chi2_contingency(\n    matrix, correction=False\n)',
        description: 'Tests association between two categorical variables using a contingency table.',
        params: ['matrix = observed frequency table', 'correction = False (disable Yates\' correction)'],
        gotcha: 'Set correction=False for tables larger than 2×2 or when all expected counts ≥ 5.',
      },
      {
        name: 'Standardized Residuals (Chi-Square)',
        code: 'std_resid = (matrix - expected) / np.sqrt(expected)',
        description: 'Identifies which cells drive the association. |value| > 2 indicates significant deviation.',
      },
    ],
  },
  {
...
export const formulaCategories: readonly FormulaCategory[] = [
  ...
  },
  {
    ...
    description: 'Comparing means across 3+ groups',
    formulas: [
      {
        name: 'F-Statistic',
        formula: 'F = MSG / MSW',
        description: 'Ratio of between-group variance to within-group variance. Large F → groups differ.',
        variables: ['MSG = Mean Square between Groups = SSG / (k − 1)', 'MSW = Mean Square Within = SSW / (N − k)', 'k = number of groups', 'N = total observations'],
        pythonCode: 'stats.f_oneway(group1, group2, group3)',
      },
      {
        name: 'Total Sum of Squares',
        formula: 'SST = SSG + SSW = Σ(xᵢ − x̄)²',
        description: 'Total variation in the data, partitioned into between-group and within-group components.',
        variables: ['SST = total sum of squares', 'SSG = sum of squares between groups', 'SSW = sum of squares within groups'],
      },
      {
        name: 'Sum of Squares Between Groups',
        formula: 'SSG = Σ nⱼ(x̄ⱼ − x̄)²',
        description: 'Variation explained by the grouping variable. How far each group mean is from the grand mean.',
        variables: ['nⱼ = size of group j', 'x̄ⱼ = mean of group j', 'x̄ = grand mean (mean of all data)'],
      },
      {
        name: 'Sum of Squares Within Groups',
        formula: 'SSW = Σ Σ (xᵢⱼ − x̄ⱼ)²',
        description: 'Unexplained variation (error). How far each observation is from its own group mean.',
        variables: ['xᵢⱼ = observation i in group j', 'x̄ⱼ = mean of group j'],
      },
      {
        name: 'Degrees of Freedom (ANOVA)',
        formula: 'df_between = k − 1,  df_within = N − k,  df_total = N − 1',
        description: 'Between-group df depends on the number of groups; within-group df depends on total observations minus groups.',
        variables: ['k = number of groups', 'N = total number of observations'],
      },
      {
        name: 'ANOVA Hypotheses',
        formula: 'H₀: μ₁ = μ₂ = … = μₖ    H₁: At least one μᵢ differs',
        description: 'A significant result (p < α) only tells you "not all means are equal" — it does NOT tell you which pairs differ. Use Tukey HSD for pairwise comparisons.',
        variables: ['μᵢ = population mean of group i', 'k = number of groups'],
        gotcha: '"Reject H₀" does NOT mean all means differ — just that at least one does.',
      },
    ],
  },
  {
    ...
    formulas: [
      {
        ...
        description: 'Measures how far observed frequencies deviate from expected frequencies under independence.',
        variables: ['Oᵢ = observed frequency in cell i', 'Eᵢ = expected frequency = (row total × col total) / grand total'],
        pythonCode: 'chi2_contingency(matrix, correction=False)',
      },
      {
        name: 'Degrees of Freedom (Chi-Square)',
        formula: 'df = (r − 1)(c − 1)',
        description: 'Depends on the number of rows (r) and columns (c) in the contingency table.',
        variables: ['r = number of rows', 'c = number of columns'],
      },
      {
        name: 'Standardized Residual',
        formula: 'SR = (O − E) / √E',
        description: 'Identifies which cells drive the overall association. |SR| > 2 indicates a significant deviation.',
        variables: ['O = observed count', 'E = expected count', '|SR| > 2 = significant cell'],
        pythonCode: '(matrix - expected) / np.sqrt(expected)',
      },
      {
        name: 'Chi-Square Hypotheses',
        formula: 'H₀: No association (variables are independent)    H₁: Association exists',
        description: 'Tests whether knowing the value of one categorical variable gives information about the other.',
        variables: [],
        gotcha: 'Use correction=False for tables larger than 2×2. Yates\' correction only for 2×2 with expected count < 5.',
      },
    ],
  },
  {
